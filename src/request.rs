...
// it seems FUSE_BIG_WRITES doesn't need 7.9 and can works well
const INIT_FLAGS: u32 = FUSE_ASYNC_READ | FUSE_POSIX_LOCKS | FUSE_BIG_WRITES;
    ...
    pub fn dispatch<FS: Filesystem>(&self, se: &mut Session<FS>) {
        ...
        match self.request.operation() {
            ...
            ll::Operation::Interrupt { arg } => {
                se.filesystem.interrupt(self, arg.unique, self.reply())
            }
            ...
            ll::Operation::Unknown { .. } => ReplyEmpty::error(self.reply(), ENOSYS),
